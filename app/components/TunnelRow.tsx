...
export function TunnelRow({ tunnel, publicBase }: TunnelRowProps) {
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-4 border-b border-white/5 px-4 py-3.5 transition-colors hover:bg-white/[0.03] last:border-b-0"
    >
      <span
        className={cn(
          "size-2 shrink-0 rounded-full",
          tunnel.live ? "bg-accent shadow-[0_0_8px_rgba(255,255,255,0.45)]" : "bg-white/25"
        )}
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        <span className="flex items-center gap-1.5 truncate font-mono text-xs text-white/35">
          <HugeiconsIcon icon={Link01Icon} size={12} color="currentColor" />
          {url}
        </span>
      </div>

      <div className="flex shrink-0 items-center gap-6 text-sm text-white/45">
        <span className={tunnel.live ? "text-accent" : "text-white/35"}>
